...
interface EventMessagesDialogProps {
  open: boolean;
...
export function EventMessagesDialog({
  ...
  useEffect(() => {
    if (open && report && facultyId) {
      loadMessages();
    }
  }, [open, report, facultyId]);
  ...
  const loadMessages = async () => {
    ...
    try {
      const response = await fetch(
        `/api/messages?facultyId=${encodeURIComponent(facultyId || "")}&entityType=event-report&entityId=${encodeURIComponent(report.id)}`,
      );
  ...
  const handleSendReply = async () => {
    if (!replyText.trim() || !report || !facultyId || !user?.id) {
    ...
    try {
      await sendMessage({
        ...
        entityId: report.id,
        threadId: `event-report:${report.id}`,
        senderRole: "faculty",
  ...
  const handleClearMessages = async () => {
    if (userRole !== "admin" || !report) return;

    if (
      !window.confirm(
        "Are you sure you want to clear all messages for this event report? This cannot be undone.",
      )
    ) {
      return;
    }
    ...
    setIsSending(true);
    try {
      const threadId = `event-report:${report.id}`;
      const response = await fetch(
